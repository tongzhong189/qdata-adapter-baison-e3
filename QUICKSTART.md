# BaisonE3Adapter 快速开始

> 5 分钟上手 baison-e3 适配器
...
```bash
git clone https://github.com/qeasy/qdata-adapter-baison-e3.git
cd qdata-adapter-baison-e3
pip install -e ".[dev]"
```
...
```bash
# standard 接口认证
BAISON_E3_CLIENT_ID=your-client-id
BAISON_E3_CLIENT_SECRET=your-client-secret
...
# 调用平台特有 API
result = await adapter.invoke(
    method="baison_e3.goods.batchupdateflag",
    object_type="goods",
...
# 运行特定测试
pytest tests/test_adapter.py::TestBaisonE3Adapter::test_authenticate -v
...
**遇到问题？** 请提交 [GitHub Issue](https://github.com/qeasy/qdata-adapter-baison-e3/issues)